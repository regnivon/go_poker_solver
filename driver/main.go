...
func main() {
	...
	tree := solv.ConstructTree(40, 400, 1, bets, ip, oop)
	solv.OutputTree(tree, 0)
	traversal := solv.NewTraversal(oop, ip)
	solv.Train(traversal, 1000, tree)


	average := tree.GetAverageStrategy()
	average2 := tree.GetNext(0).GetNext(1).(*solv.GameNode).GetAverageStrategy()

	for hand := range oop {
		fmt.Println(oop[hand].Hand, average[hand])
	}

fmt.Println("\n\nnext\n\n")
	for hand := range oop {
//fmt.Println(oop[hand].Hand, average[hand])		
	fmt.Println(oop[hand].Hand, tree.GetNext(0).GetNext(1).(*solv.GameNode).Strategy(hand), average2[hand])
	}

	//fmt.Printf("%v\n", average)
	//fmt.Printf("%v\n", average2)
	//fmt.Printf("%v %v\n", len(oop), len(ip)) 
	fmt.Println(tree.GetNext(0).GetNext(1).(*solv.GameNode).PotSize()) 
